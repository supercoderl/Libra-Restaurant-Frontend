import { orders } from "@/api/business/orderApi";
import { OrderContainer } from "@/containers/dashboard-container/orders";
...
const Order: NextPage = () => {
    ...
    const onLoad = async () => {
        setLoading(true);
        try {
            const res = await orders({ page: index, pageSize: size, searchTerm: searchText });
            if (res && res.success) {
                setResult(res.data);
            }
        }
        catch (error) {
            console.log("OnLoad Order: ", error);
        }
        finally {
            setTimeout(() => {
                setLoading(false);
